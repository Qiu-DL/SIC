...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'ViT-B/32':
        ...
        backbone, preprocess = clip.load("ViT-B-32.pt", device=device)  # ViT-B-32.pt download link:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] in ['imagenet_10', 'imagenet_dog', 'imagenet_tiny', 'imagenet_50', 'imagenet_100',
                                'imagenet_200']:
    ...
    if to_neighbors_dataset: # Dataset returns an image and one of its nearest neighbors.
        ...
        indices = np.load(os.path.join('/log', p['train_db_name'], 'train_knn_indices.npy'))
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] in ['imagenet_10', 'imagenet_dog', 'imagenet_tiny', 'imagenet_50', 'imagenet_100',
                              'imagenet_200']:
        from data.imagenet import ImageNetSubset
    ...
    if to_neighbors_dataset: # Dataset returns an image and one of its nearest neighbors.
        from data.custom_dataset import NeighborsDataset
        indices = np.load(os.path.join('/log', p['train_db_name'], 'val_knn_indices.npy'))
...
def construct_semantic_space(p, image_centers, model, args):
    ...
    if not os.path.exists(os.path.join('/log', p['train_db_name'], 'target_list.pth.tar')):
        ...
        torch.save(target_list, os.path.join('/log', p['train_db_name'], 'target_list.pth.tar'))
    else:
        target_list = torch.load(os.path.join('/log', p['train_db_name'], 'target_list.pth.tar'))
