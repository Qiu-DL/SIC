...
def get_knn_indices(model, train_dataloader, val_dataloader, topk):
    ...
    train_indices, train_accuracy = mine_nearest_neighbors(train_features.numpy(), train_targets.cpu().numpy(), topk+1)
    val_indices, val_accuracy = mine_nearest_neighbors(val_features.numpy(), val_targets.cpu().numpy(), 5+1)
